...
export class RecommendationEngine {
  ...
  private getWorkflowRecommendations(): Recommendation[] {
    ...
    if (stats.byCategory['code_execution'] === undefined || stats.byCategory['code_execution'] === 0) {
      recommendations.push({
        type: 'workflow',
        priority: 'high',
        title: 'Enable MCP Code Execution',
        description: 'MCP code execution can reduce token usage by up to 98.7%.',
    ...
    if (stats.byCategory['skills'] && stats.byCategory['skills'] > 10000) {
      recommendations.push({
        ...
        description: 'Progressive skill loading can reduce skill-related token usage by 95%.',
   ...
   */
  private calculatePriority(savings: number): 'low' | 'medium' | 'high' | 'critical' {
