...
export class ExecutionOrchestrator {
  ...
  async execute(
    ...
    try {
      ...
      console.log(`✓ Generated ${wrapper.estimatedTokens} token code wrapper\n`);
      ...
      console.log('Phase 4: Sandbox Execution - Running in isolated environment...');
      const workspace = await this.workspaceManager.createWorkspace(wrapper.code, language);
      this.workspaceManager.updateStatus(workspace.id, 'running');

      const sandboxConfig = this.getDefaultSandboxConfig();
      const result = await this.sandboxManager.execute(wrapper.code, language, sandboxConfig);
      ...
      console.log('Phase 5: Result Processing - Tokenizing PII and summarizing...');

      // Tokenize PII in output
      if (result.output && this.tokenizer.containsPII(JSON.stringify(result.output))) {
   ...
   * Create error result
   */
  private createErrorResult(error: string, metadata?: Record<string, any>): ExecutionResult {
    return {
      success: false,
      error,
